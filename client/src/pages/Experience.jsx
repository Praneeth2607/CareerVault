  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-heading font-bold text-heading">Work Experience</h2>
          <p className="text-heading/70 mt-2">Track your professional history, roles, and achievements.</p>
        </div>
        <button 
          onClick={() => handleOpenModal()} 
          className="flex items-center px-4 py-2 bg-primary text-bg-secondary rounded-button shadow-md hover:bg-primary/90 hover:-translate-y-0.5 transition-all font-medium"
        >
          <Plus className="w-5 h-5 mr-2" /> Add Experience
        </button>
      </div>

      <AssetList key={listKey} assetType="WORK_EXPERIENCE" onEdit={handleOpenModal} />
          ...
          <div className="bg-bg-secondary w-full max-w-2xl rounded-modal shadow-raised max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in-95 duration-200">
            <div className="sticky top-0 bg-bg-secondary/90 backdrop-blur-md p-6 border-b border-heading/5 flex justify-between items-center z-10">
              <h3 className="text-2xl font-heading font-bold text-primary">
                {editingAsset ? 'Edit Experience' : 'New Experience'}
              </h3>
